...
$$\int f dG, \int f(x) dG, \int ... \int f(x_1, x_2,..., x_m) dx_1dx_2...dx_m$$
...
$$\int f dG = \int fd(G' \cup G'') =\int f dG' + \int f dG'' $$
\end{theorem}
...
$$\iint_G f(x,y) dx dy = \int_a ^b \left( \int_{\varphi(x)} ^{\psi(x)}  f(x,y) dy \right) dx$$
\end{theorem}
...
$$\iint_G f(x,y) dx dy = \int_c ^d dy  \int_{\alpha(y)} ^{\beta(y)}  f(x,y) dx$$

\begin{example}
$z = x^2y$ заградена между $y = x^2, y = 1$. 
Имаме: $\iint_G x^2y \,dx dy$, 
$G =
\begin{cases}
y = x^2 \\
y = 1
...
Тогава 
\begin{gather*}
\iint_G x^2y \,dxdy = \int_{-1} ^ 1 x^2dx \int _{x^2} ^ 1 y dy \qquad \left( \int _{x^2} ^ 1 y dy  = \frac{y^2}{2}  \Big|_{x^2} ^ 1\right) \\
